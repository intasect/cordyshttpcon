...
/**
 * This class is the Implementation of ApplicationTransaction. This class will
 * receive the request process it if it is a valid one.
 */
public class HttpTransaction implements ApplicationTransaction {
	/**
	 * Contains the logger instance.
	 */
	private static CordysLogger LOG = CordysLogger
			.getCordysLogger(HttpTransaction.class);
	/**
	 * The request type by which the request is to be redirected to different
	 * classes.
	 */
	 ...
	 */
	public HttpTransaction(HttpConnector acAppConnector,
			HttpConfiguration acConfig) {
	 ...
	 * 
	 * @param bbRequest
	 *            The request body block.
	 * @param bbResponse
	 *            The response body block.
	 * 
	...
	public boolean process(BodyBlock bbRequest, BodyBlock bbResponse) {
		...
		try {
			IMethodConfiguration methodInfo = acAppConnector
					.getMethodConfig(bbRequest);
			ConnectionManager factory = acAppConnector.getConnectionFactory();
			IServerConnection serverConnection = factory
					.getConnection(methodInfo.getConnectionId());
			if (serverConnection == null) {
				throw new ConnectorException(
						ConnectorExceptionMessages.NO_CONNECTION_FOUND_WITH_ID_0,
						methodInfo.getConnectionId());
			}

			// Get the data XML node and unlink it, so that the XSLT can work
			// properly.
			reqNode = Node.unlink(bbRequest.getXMLNode());

			// Send the HTTP request.
			int resNode = MethodExecutor.sendRequest(reqNode, serverConnection,
					methodInfo, acAppConnector.getCounters());
			...
			if (resNode != 0) {
				...
				if (resFirstChild != 0) {
					...
					Map<String, String> rootNamespaces = XmlUtils
							.getNamespaceDeclarations(resNode);

					XmlUtils.addNamespaceDeclarations(responseMethodNode,
							rootNamespaces);
					Node.appendToChildren(resFirstChild, resLastChild,
							responseMethodNode);

					for (int tmpNode = Node.getFirstChild(responseMethodNode); tmpNode != 0;) {
						XmlUtils.removeNamespaceDeclarations(tmpNode,
								rootNamespaces);
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Sending SOAP response: "
						+ Node.writeToString(responseEnvelope, true));
			}
		} catch (HandlerException he) {
			// Either the request/response handler threw an exception, so we
			...
			// ServerLocalizableException. That message we will use to send as
			// the main message to
			// the end user. If no SLE could be found the current exception is
			// wrapped into a new
			// SLE exception.
			int iMaxHop = 30;
			int iCurrentHop = 0;
			...
			while ((tCurrent != null) && (iCurrentHop < iMaxHop)) {
				...
				// The whole hop-thing is to make sure that if there are cyclic
				// references in the
