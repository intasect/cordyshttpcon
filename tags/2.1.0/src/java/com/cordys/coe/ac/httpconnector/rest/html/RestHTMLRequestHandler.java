...
public class RestHTMLRequestHandler extends StandardRequestHandler
{
     ...
     */
    private static final CordysLogger LOG = CordysLogger.getCordysLogger(RestHTMLRequestHandler.class);
    /**
                              ...
                              throws HandlerException
    {
        ...
        int[] parameters = XPathHelper.selectNodes(configXml,
                                                   "ns:" + TAG_PARAMETERS + "/ns:" + TAG_PARAMETER,
                                                   xmi);
    ...
    /**
     * @see  com.cordys.coe.ac.httpconnector.impl.StandardRequestHandler#getPostData(int, com.cordys.coe.ac.httpconnector.config.IServerConnection)
     */
    @Override protected byte[] getPostData(int requestNode, IServerConnection connection)
                                    throws HandlerException
    {
        ...
        for (IRestHTMLParameter rhp : m_bodyParameters.values())
        {
            ...
            {
                int[] matches = XPathHelper.selectNodes(requestNode, rhp.getValue(),
                                                        m_method.getXPathMetaInfo());
            ...
            else if (rhp.getType() == ERestHTMLParameterType.CUSTOM)
            {
                ...
                try
                {
                    ...
                    ICustomParameter cp = (ICustomParameter) clazz.newInstance();
                    String additionalData = cp.getPostData(rhp, requestNode, m_method, connection);
            ...
            else
            {
                ...
                switch (rhp.getType())
                {
                    ...
                    case CONNECTION_PARAMETER:
                        ...
                        if (connParam != null)
                        {
                            // In this case the XML parameters are not used since we cannot put XML
                            // in the request.
                ...
                if ((value == null) || (value.length() == 0))
                {
                    ...
                    {
                        throw new HandlerException(HandlerExceptionMessages.THE_PARAMETER_0_HAS_NO_VALUE,
                                                   rhp.getName());
                    }

                    // Now we need to decide whether or not we're going to include this parameter
                    ...
                    {
                        // It's an optional parameter without a default parameter. Ignore it.
                        include = false;
                    }
                }
        ...
        if ((uri != null) && (uri.length() > 0))
        {
            ...
            for (IRestHTMLParameter rhp : m_requestParameters.values())
            {
                ...
                if (value == null)
                {
                    if (rhp.getOptional() == false)
                    {
                        throw new HandlerException(HandlerExceptionMessages.FOR_PARAMETER_0_NO_VALUE_COULD_BE_FOUND,
                                                   rhp.getName());
                    }
