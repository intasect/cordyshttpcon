...
import com.cordys.coe.ac.httpconnector.config.DefaultConnectionManager;
...
public class TestJIRACalls {
	 ...
	 */
	private CordysLogger LOG = CordysLogger
			.getCordysLogger(TestJIRACalls.class);
	/**
	...
	public void setup(String method) throws Exception {
		LoggerConfigurator
				.initLogger("../InboudEmailConnector_main/test/Log4jConfiguration.xml");
		...
		int connection = s_doc.parseString(FileUtils.readTextResourceContents(
				"connections.xml", getClass()));
		m_sc = DefaultConnectionManager.createServerConnections(connection)[0];
		m_sc.open();
		...
		int methodImpl = s_doc.parseString(FileUtils.readTextResourceContents(
				method + "-impl.xml", getClass()));
		m_mc = MethodConfigurationFactory.createMethodConfiguration(
				new LocalHTTPConnector(), methodImpl);

		m_request = s_doc.parseString(FileUtils.readTextResourceContents(method
				+ "-request.xml", getClass()));
	}
	 ...
	 * @author pgussow
	 */
	public class LocalHTTPConnector implements IXSLTStore {
		/**
