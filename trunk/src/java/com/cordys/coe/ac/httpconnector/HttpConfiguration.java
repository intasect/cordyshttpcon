...
public class HttpConfiguration
{
     ...
     */
    private static CordysLogger LOG = CordysLogger.getCordysLogger(HttpConfiguration.class);
    /**
     ...
     */
    public HttpConfiguration(int iConfigNode)
                      throws ConnectorException
    {
        if (iConfigNode == 0)
        {
            throw new ConnectorException(ConnectorExceptionMessages.CONFIGURATION_NOT_FOUND);
        }

        if (LOG.isDebugEnabled())
        {
            LOG.debug("The current configuration:\n" + Node.writeToString(iConfigNode, false));
        }

        if (!Node.getName(iConfigNode).equals(TAG_CONFIGURATION_LC))
        {
            throw new ConnectorException(ConnectorExceptionMessages.ROOTTAG_OF_THE_CONFIGURATION_SHOULD_BE_CONFIGURATION);
        }

        // Create XPath meta info
        XPathMetaInfo xmi = new XPathMetaInfo();
        xmi.addNamespaceBinding("ns", HttpConnectorConstants.NS_HTTP_CONNECTOR_CONF_2_0);

        int child = XPathHelper.selectSingleNode(iConfigNode, "ns:" + TAG_CONFIGURATION, xmi);

        if ((child == 0) || !Node.getName(child).equals(TAG_CONFIGURATION))
        {
            throw new ConnectorException(ConnectorExceptionMessages.CONFIGURATION_ELEMENT_NOT_FOUND);
        }

        // Get the path to the configuration file.
        m_configPath = XPathHelper.getStringValue(child, "//ns:" + TAG_CONFIG_PATH, xmi, "");

        if (m_configPath.length() == 0)
        {
            throw new ConnectorException(ConnectorExceptionMessages.THE_TAG_CONFIGPATH_MUST_BE_FILLED);
        }
    }
