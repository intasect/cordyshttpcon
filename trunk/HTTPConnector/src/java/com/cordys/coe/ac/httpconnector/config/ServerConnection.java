...
import static com.cordys.coe.ac.httpconnector.HttpConnectorConstants.HTTPS_PROTOCOL_PREFIX;
...
import com.cordys.security.certificatemanager.CertificateManager;
import com.cordys.security.exceptions.CertificateManagementException;
import com.cordys.security.ssl.apache.CordysApacheSSLSocketFactory;
...
class ServerConnection implements IServerConnection {
	...
	/**
	 * If <code>true</code> authentication information is always sent before the
	 * server asks for it.
	 */
	...
	private static final CordysLogger LOG = CordysLogger
			.getCordysLogger(ServerConnection.class);
	 ...
	 *            The connection XML.
	 * @param xmi
	 *            The proper XMI to use with prefix ns bound to the proepr
	 *            namespace.
	 * 
	...
	ServerConnection(int connection, XPathMetaInfo xmi)
		...
		if ((m_id == null) || (m_id.length() == 0)) {
			throw new ConnectorException(
					ConnectorExceptionMessages.CONNECTION_ATTRIBUTE_ID_IS_MISSING);
		}
		...
		if (tmpStr.length() == 0) {
			throw new ConnectorException(
					ConnectorExceptionMessages.CONNECTION_ELEMENT_URL_IS_MISSING);
		}
		...
		try {
			...
			throw new ConnectorException(e1,
					ConnectorExceptionMessages.INVALID_SERVER_URL_0, tmpStr);
		}

		// Get the optional timeout
		m_timeout = XPathHelper.getIntegerValue(connection,
				"ns:" + TAG_TIMEOUT, xmi, DEFAULT_TIMEOUT);

		m_username = XPathHelper.getStringValue(connection, "ns:"
				+ TAG_USERNAME, xmi, null);
		m_password = XPathHelper.getStringValue(connection, "ns:"
				+ TAG_PASSWORD, xmi, null);

		// Decode the Base64 encoded password.
		if (m_password != null) {
			byte[] bytes = m_password.getBytes();
			m_password = new String(new Base64Encoding().decode(bytes));
		}

		m_proxyHost = XPathHelper.getStringValue(connection, "ns:"
				+ TAG_PROXY_HOST, xmi, null);

		if ((m_proxyHost != null) && (m_proxyHost.length() > 0)) {
			tmpStr = XPathHelper.getStringValue(connection, "ns:"
					+ TAG_PROXY_PORT, xmi, null);

			if ((tmpStr == null) || (tmpStr.length() == 0)) {
				throw new ConnectorException(
						ConnectorExceptionMessages.PROXY_SERVER_PORT_MUST_BE_SET);
			}
			...
			m_proxyUsername = XPathHelper.getStringValue(connection, "ns:"
					+ TAG_PROXY_USERNAME, xmi, null);
			m_proxyPassword = XPathHelper.getStringValue(connection, "ns:"
					+ TAG_PROXY_PASSWORD, xmi, null);

			if (m_proxyPassword != null) {
				byte[] bytes = m_proxyPassword.getBytes();
				m_proxyPassword = new String(new Base64Encoding().decode(bytes));
			}
		}

		m_checkServerCertificate = XPathHelper.getBooleanValue(connection,
				TAG_CHECK_CERTIFICATE, xmi, true);
		m_authenticateAlways = XPathHelper.getBooleanValue(connection,
				TAG_AUTHENTICATE_ALWAYS, xmi, false);
	...
	public void open() {
		...
		if (HTTPS_PROTOCOL_PREFIX.equals(protoName)) {
			if (m_checkServerCertificate) {
				...
				protocol = new Protocol(HTTPS_PROTOCOL_PREFIX,
				...
				protocol = new Protocol("https",
						(ProtocolSocketFactory) new DummySSLSocketFactory(),
						port);
		...
		if (m_proxyHost != null) {
			...
			if (m_proxyUsername != null) {
				...
				m_client.getState().setProxyCredentials(
		...
		if (m_username != null) {
			...
			m_client.getState().setCredentials(
					new AuthScope(hostName, port, AuthScope.ANY_REALM),
					defaultcreds);
		}
	}

	private ProtocolSocketFactory createSocketFactory() {
		ProtocolSocketFactory socketFactory = null;
		try {
			CertificateManager certificateManager = CertificateManager
					.initializeAndReturnNewInstance(null, false);
			socketFactory = new CordysApacheSSLSocketFactory(certificateManager);
		} catch (CertificateManagementException e) {
			throw new LocalizableRuntimeException((Exception) e,
					ConnectorExceptionMessages.FAILED_TO_CREATE_SOCKET_FACTORY,
					e.getMessage());
		}
