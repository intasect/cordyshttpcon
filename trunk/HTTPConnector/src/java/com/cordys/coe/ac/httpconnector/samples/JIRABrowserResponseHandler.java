...
/**
 * This class gets the users from the response and puts them into an XML
 * structure.
 * 
 * @author pgussow
 */
public class JIRABrowserResponseHandler extends JIRAResponseHandler {
	...
	private static final CordysLogger LOG = CordysLogger
			.getCordysLogger(JIRABrowserResponseHandler.class);
	...
	protected void buildXMLResponse(int resNode, HttpMethod httpMethod,
		...
		NodeList nodeList = (NodeList) xpath.evaluate(
				"//table[@class='grid']/tr[@class='vcard']", document,
		...
		if (nrOfUsers > 0) {
			for (int count = 0; count < nrOfUsers; count++) {
				...
				// Get the name
				String username = (String) xpath.evaluate(
						".//span[@class='username']/text()", userNode,
						XPathConstants.STRING);

				doc.createElementWithParentNS("username", username, user);

				// Get the email address
				String emailAddress = (String) xpath.evaluate(
						".//span[@class='email']/text()", userNode,
						XPathConstants.STRING);
				...
				NodeList nl = (NodeList) xpath.evaluate(
						".//td/a[../br]/text()", userNode,
						XPathConstants.NODESET);
				int groups = doc
						.createElementWithParentNS("groups", null, user);
