...
/**
 * This class implements the connection parameters that can be freely configured.
 *
 * @author  pgussow
 */
class Parameter
    implements IParameter
{
     ...
     * @param   xmi            The XPathMetaInfo.
     *
     * @throws  ConnectorException  In case of any exceptions.
     */
    Parameter(int configuration, XPathMetaInfo xmi)
       throws ConnectorException
    {
        // Parse and check the name of the parameter
        m_name = XPathHelper.getStringValue(configuration, "ns:" + TAG_NAME, xmi, "");

        if (m_name.length() == 0)
        {
            throw new ConnectorException(ConnectorExceptionMessages.THE_NAME_OF_THE_PARAMETER_MUST_BE_FILLED);
        }

        // Parse the type.
        String type = Node.getAttribute(configuration, ATTRIBUTE_TYPE,
                                        EParameterType.STRING.name());
        m_type = EParameterType.valueOf(type.toUpperCase());
        ...
        switch (m_type)
        {
            ...
            case STRING:
                ...
                String tempString = XPathHelper.getStringValue(configuration, "ns:" + TAG_VALUE,
                                                               xmi, "");
                if (tempString.length() == 0)
                {
                    throw new ConnectorException(ConnectorExceptionMessages.THE_VALUE_FOR_PARAMETER_0_IS_EMPTY,
                                                 m_name);
                }
                m_value = tempString;
                ...
                if (decode)
                {
                    ...
                    m_value = new String(Native.decodeBinBase64(bytes, bytes.length));
                }
            ...
            case XML:
                ...
                int tempInt = XPathHelper.selectSingleNode(configuration, "ns:" + TAG_VALUE + "/*",
                                                           xmi);
                if (tempInt == 0)
                {
                    throw new ConnectorException(ConnectorExceptionMessages.THE_PARAMETER_0_HAS_NO_CHILD_ELEMENT,
                                                 m_name);
                }
                // The node is cloned. People SHOULD explicitly call the clean() method, but there
                // is a fallback scenario using the finalizer.
                m_value = (Integer) Node.duplicate(tempInt);
                break;
        }
    }
     ...
     *
     * @throws  Throwable  In case the clean fails.
     *
