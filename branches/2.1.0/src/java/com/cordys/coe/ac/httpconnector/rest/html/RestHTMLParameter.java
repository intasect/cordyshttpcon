 ...
 * @author  pgussow
 */
class RestHTMLParameter
    implements IRestHTMLParameter
{
    /**
     ...
     *
     * @param   parameter  The parameter XML.
     * @param   xmi        The XPathMetaInfo object with the ns prefix mapped to the proper
     *                     namespace.
     *
     * @throws  HandlerException  In case of any exceptions.
     */
    RestHTMLParameter(int parameter, XPathMetaInfo xmi)
               throws HandlerException
    {
        ...
        if (m_name.length() == 0)
        {
            throw new HandlerException(HandlerExceptionMessages.FOR_THE_RESTHTMLREQUESTHANDLER_THE_TAG_NAME_MUST_BE_FILLED);
        }

        // Get whether or not the parameter is optional.
        m_optional = XPathHelper.getBooleanValue(parameter, "ns:" + "optional", xmi, false);

        // Get the destination.
        String temp = XPathHelper.getStringValue(parameter, "ns:" + "destination", xmi, "")
                                 .toUpperCase();

        if (temp.length() == 0)
        {
            throw new HandlerException(HandlerExceptionMessages.FOR_THE_RESTHTMLREQUESTHANDLER_THE_TAG_DESTINATION_MUST_BE_FILLED);
        }
        ...
        {
            throw new HandlerException(HandlerExceptionMessages.THE_VALUE_MUST_BE_FILLED_SINCE_THIS_IS_A_MANDATORY_PARAMETER, m_name);
        }

        // Get the default value
        m_defaultValue = XPathHelper.getStringValue(parameter, "ns:" + "default", xmi, null);

        if (m_defaultValue == null)
        {
            int tmp = XPathHelper.selectSingleNode(parameter, "ns:" + "default", xmi);
