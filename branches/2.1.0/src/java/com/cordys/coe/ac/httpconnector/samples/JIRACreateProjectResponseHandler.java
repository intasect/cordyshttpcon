...
/**
 * This class handles the response from the create project. the most important thig is to figure out
 * the project ID.
 *
 * @author  pgussow
 */
public class JIRACreateProjectResponseHandler extends JIRAResponseHandler
{
     ...
     */
    private static final Pattern PATTERN_PID = Pattern.compile("^http://.+/secure/admin.user/ViewProject\\.jspa\\?pid=(\\d+)$");
     ...
     */
    @Override protected void buildXMLResponse(int resNode, HttpMethod httpMethod, Document document,
                                              com.eibus.xml.nom.Document doc)
                                       throws Exception
    {
        if (httpMethod.getStatusCode() == 302)
        {
            // It's a redirect to the details screen for the newly created project. We'll follow
            // the redirect.
