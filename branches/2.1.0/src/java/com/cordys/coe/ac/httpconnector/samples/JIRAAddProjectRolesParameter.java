...
/**
 * This class makes the complex post data for adding project roles to a certain user.
 *
 * @author  pgussow
 */
public class JIRAAddProjectRolesParameter
    implements ICustomParameter
{
    /**
     * @see  ICustomParameter#getPostData(IRestHTMLParameter, int, IMethodConfiguration,
     *       IServerConnection)
     */
    @Override public String getPostData(IRestHTMLParameter rhp, int requestNode,
                                 ...
                                 throws HandlerException
    {
        ...
        int[] projects = XPathHelper.selectNodes(requestNode, "ns:projects/ns:project",
                                                 methodConfiguration.getXPathMetaInfo());

        for (int project : projects)
        {
            String projectid = XPathHelper.getStringValue(project, "ns:projectid",
                                                          methodConfiguration.getXPathMetaInfo(),
                                                          "");

            if (projectid.length() == 0)
            {
                throw new HandlerException(HandlerExceptionMessages.MISSING_PROJECT_ID);
            }
            ...
            if (roles.length > 0)
            {
                ...
                for (int role : roles)
                {
                    String roleid = XPathHelper.getStringValue(role, "ns:id",
                                                               methodConfiguration
                                                               .getXPathMetaInfo(), "");
                    String newValue = XPathHelper.getStringValue(role, "ns:new",
                                                                 methodConfiguration
                                                                 .getXPathMetaInfo(), "");
                    String oldValue = XPathHelper.getStringValue(role, "ns:old",
                                                                 methodConfiguration
                                                                 .getXPathMetaInfo(), "");

                    if (roleid.trim().length() == 0)
                    {
                        throw new HandlerException(HandlerExceptionMessages.THE_ROLE_ID_MUST_BE_FILLED);
                    }

                    if (newValue.trim().length() == 0)
                    {
                        throw new HandlerException(HandlerExceptionMessages.THE_ROLE_VALUE_MUST_BE_FILLED);
                    }

                    if (oldValue.trim().length() == 0)
                    {
                        throw new HandlerException(HandlerExceptionMessages.THE_OLD_ROLE_VALUE_MUST_BE_FILLED);
                    }

                    boolean turningOn = newValue.toLowerCase().equals("on");
                    boolean wasTurnedOn = oldValue.toLowerCase().equals("on");
                    ...
                    // Add the orig (for now always the opposite to make sure they are really set.
                    returnValue.append("&").append(postVarname).append("_orig=");
