    ...
    implements IResponseHandler
{
     ...
     */
    private static final CordysLogger LOG = CordysLogger.getCordysLogger(StandardResponseHandler.class);
    /**
    ...
    /**
     * Contains an optional response XSLT object.
     */
    protected XSLT responseXslt;
    /**
    ...
    /**
     * @see  IResponseHandler#convertResponseToXml(HttpMethod, IServerConnection, Document)
     */
    public int convertResponseToXml(HttpMethod httpMethod, IServerConnection serverConnection,
                                    Document doc)
                             throws XMLException, ConnectorException, IOException
    {
        ...
        try
        {
            ...
            if (validResponseXPath != null)
            {
                ...
                {
                    throw new ConnectorException(ConnectorExceptionMessages.THE_RESPONSE_XML_DID_NOT_MATCH_THE_VALID_RESPONSE_XPATH_THAT_WAS_CONFIGURED_IN_THE_METHOD_IMPLEMENTATION);
                }
            }
            ...
            {
                XmlUtils.removeNamespacesRecursively(resNode, m_removeNamespaceUriSet);
            }
    ...
    /**
     * @see  IResponseHandler#initialize(int, IXSLTStore, IMethodConfiguration, XPathMetaInfo)
     */
    @Override
                    ...
                    throws ConnectorException
    {
        ...
        {
            responseXslt = connector.loadXslt(tmpNode);
        }
     ...
     *
     * @param   httpMethod  HTTP method object from which the response data is read.
     * @param   doc         NOM document for creating the XML.
     *
     ...
     *
     * @param   responseNode  Response node to be transformed.
    ...
    {
        int newNode = responseXslt.xslTransform(responseNode, method.getXPathMetaInfo());
     ...
     *
     * @throws  IOException  In case the response could not be read for some reason.
     */
    protected byte[] getHTTPResponse(HttpMethod httpMethod)
                              throws IOException
    {
    ...
    /**
     * Method which can do processing of the response XML after the XSLT transform is executed. If a
     * new node is returned, the parameter node must be deleted by this method.
     *
     * <p>This implementation just returns the node as-is.</p>
     *
    ...
    /**
     * Method which can do processing of the response XML before the XSLT transform is called. If a
     * new node is returned, the parameter node must be deleted by this method.
     *
     * <p>This implementation just returns the node as-is.</p>
     *
     ...
     */
    protected int preProcessXml(int reqNode)
                         throws ConnectorException
    {
        return reqNode;
    }
