...
/**
 * This class actually executes the request. This class decouples the
 * application connector framework from the actual execution of the HTTP
 * request.
 * 
 * @author pgussow
 */
public class MethodExecutor {
	...
	private static final CordysLogger LOG = CordysLogger
			.getCordysLogger(MethodExecutor.class);
	 ...
	 */
	public static int sendRequest(int reqNode,
			IServerConnection serverConnection, IMethodConfiguration methodInfo)
			throws ConnectorException, HandlerException {
		return sendRequest(reqNode, serverConnection, methodInfo, null);
	}
	...
	public static int sendRequest(int reqNode,
			...
			IMethodConfiguration methodInfo, PerformanceCounters counters)
			throws ConnectorException, HandlerException {
		...
		try {
			...
			int responseNode = responseHandler.convertResponseToXml(httpMethod,
					serverConnection, Node.getDocument(reqNode));
			if (setJMXInfo) {
			...
			if ((validStatusCode >= 0) && (statusCode != validStatusCode)) {
				...
				throw new ConnectorException(
						ConnectorExceptionMessages.INVALID_STATUS_CODE_RECEIVED_0_EXPECTED_1,
			...
			throw new ConnectorException(e,
					ConnectorExceptionMessages.HTTP_REQUEST_FAILED_0,
					e.getMessage());
		} catch (IOException e) {
			throw new ConnectorException(e,
					ConnectorExceptionMessages.HTTP_CONNECTION_FAILED_0,
					e.getMessage());
