...
/**
 * Standard HTTP connector request handler. This will execute the XSLT
 * transformation, if configured
 * 
 * @author mpoyhone
 */
public class StandardRequestHandler implements IRequestHandler {
	/**
	...
	private static final CordysLogger LOG = CordysLogger
			.getCordysLogger(StandardRequestHandler.class);
	/**
	 ...
	 */
	@Override
	public HttpMethod process(int requestNode, IServerConnection connection,
			HttpClient httpClient) throws HandlerException {
		...
		default:
			throw new HandlerException(
					HandlerExceptionMessages.UNKNOWN_HTTP_METHOD);
		}
		...
		try {
			...
			if (m_requestRootXPath != null) {
				reqNode = handleRequestXPath(reqNode, m_method,
						reqNode != requestNode);
			}

			if ((m_removeNamespaceUriSet != null) || m_removeAllNamespaces) {
				XmlUtils.removeNamespacesRecursively(reqNode,
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Final Request XML: "
						+ Node.writeToString(reqNode, true));
			}
	 ...
	 * @throws HandlerException
	 */
	protected int executeXslt(int requestNode, IMethodConfiguration method,
			boolean mustDelete) throws HandlerException {
		XSLT xslt = this.xslt;
		try {
			if (xslt == null) {
			...
			throw new HandlerException(e, e.getMessageObject(),
	 ...
	 * @return Transformed XML.
	 */
	protected int handleRequestXPath(int requestNode,
			IMethodConfiguration method, boolean mustDelete) {
		int node = m_requestRootXPath.firstMatch(requestNode,
