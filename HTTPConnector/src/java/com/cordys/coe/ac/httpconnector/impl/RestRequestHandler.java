...
public class RestRequestHandler extends StandardRequestHandler {
	...
	private static final CordysLogger LOG = CordysLogger
			.getCordysLogger(RestRequestHandler.class);
	/**
	...
	/**
	 * @see StandardRequestHandler#initialize(int, IXSLTStore,
	...
	public void initialize(int configXml, IXSLTStore connector,
		...
		int[] params = XPathHelper.selectNodes(configXml, "ns:"
				+ TAG_URI_PARAMETERS + "/ns:" + TAG_PARAMETER, xmi);
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (p.type == null) {
				throw new HandlerException(
						HandlerExceptionMessages.INVALID_URI_PARAMETER_TYPE_0,
						typeStr);
			}
			...
			case XPATH:
				...
				if ((xpathStr == null) || (xpathStr.length() == 0)) {
					throw new HandlerException(
							HandlerExceptionMessages.PARAMETER_XPATH_IS_NOT_SET);
				}
	...
	/**
	 * @see StandardRequestHandler#getRequestUri(int, IServerConnection,
	 *      HttpClient)
	 */
	@Override
	protected String getRequestUri(int requestNode,
		...
		for (int i = 0; i < args.length; i++) {
			...
			switch (param.type) {
			case XPATH:
				value = XPathHelper.getStringValue(requestNode,
						(XPath) param.value, null);
				break;
			...
			case CONNECTION_PARAMETER:
				...
				IParameter connParam = connection
						.getParameter((String) param.value);
				if (connParam != null) {
	...
	private enum EUriParameterType {
		CONNECTION_URI("connection-uri"), XPATH("xpath"), FIXED("fixed"), CONNECTION_PARAMETER(
