 ...
 * @author mpoyhone
 */
class MethodConfiguration implements IMethodConfiguration {
	/**
	 * If <code>true</code>, the response SOAP body contents will be cleared.
	 * This is useful only for methods which cannot return the response method
	 * element.
	 */
	...
	/**
	 * Contains the HTTP request handler for converting the SOAP request XML
	 * into an HTTP request.
	 */
	private IRequestHandler m_requestHandler;
	/**
	 * Contains the HTTP response handler for converting the HTTP response to
	 * SOAP request XML.
	 */
	 ...
	 * 
	 * @param implNode
	 *            The implementation XML to parse
	 * @param connector
	 *            The http connector object.
	 * @param xmi
	 *            The proper namespace prefix mapping to use.
	 * 
	 * @throws ConnectorException
	 *             Thrown if the method could not be parsed.
	 ...
	 *            The HTTP connector.
	 * @param xmi
	 *            The namespace prefix mapping. The prefix ns must be mapped to
	 ...
	 *             In case the handler throws any exception.
	 */
	private void parseConfigurationXML(int implNode, IXSLTStore connector,
			XPathMetaInfo xmi) throws ConnectorException, HandlerException {
		...
		m_connectionId = XPathHelper.getStringValue(implNode, "ns:"
				+ TAG_CONNECTION_ID, xmi, "");

		if (m_connectionId.length() == 0) {
			throw new ConnectorException(
					ConnectorExceptionMessages.CONNECTION_ID_IS_NOT_SET);
		}
		...
		m_cleanResponseBody = XPathHelper.getBooleanValue(implNode, "ns:"
				+ TAG_CLEAN_RESPONSE_BODY, xmi, false);

		// Try to read the HTTP method type (default to POST).
		tmpStr = XPathHelper.getStringValue(implNode, "ns:" + TAG_HTTP_METHOD,
				xmi, "POST");
		m_httpMethodType = EHttpMethod.valueOf(tmpStr.toUpperCase(Locale.US).trim());

		// Load the request handler.
		if ((requestHandlerNode = XPathHelper.selectSingleNode(implNode, "ns:"
				+ TAG_REQUEST_HANDLER, xmi)) == 0) {
			throw new ConnectorException(
					ConnectorExceptionMessages.REQUEST_HANDLER_ELEMENT_IS_NOT_SET);
		}
		tmpStr = Node.getAttribute(requestHandlerNode, ATTRIBUTE_CLASS);

		if ((tmpStr == null) || (tmpStr.length() == 0)) {
			throw new ConnectorException(
					ConnectorExceptionMessages.CLASS_ATTRIBUTE_MISSING_FROM_THE_REQUEST_HANDLER_ELEMENT);
		}
		...
		if ((responseHandlerNode = XPathHelper.selectSingleNode(implNode, "ns:"
				+ TAG_RESPONSE_HANDLER, xmi)) == 0) {
			throw new ConnectorException(
					ConnectorExceptionMessages.RESPONSE_HANDLER_ELEMENT_IS_NOT_SET);
		}
		tmpStr = Node.getAttribute(responseHandlerNode, ATTRIBUTE_CLASS);

		if ((tmpStr == null) || (tmpStr.length() == 0)) {
			throw new ConnectorException(
					ConnectorExceptionMessages.CLASS_ATTRIBUTE_MISSING_FROM_THE_RESPONSE_HANDLER_ELEMENT);
		}
		...
		tmpStr = XPathHelper.getStringValue(implNode, "ns:"
				+ TAG_VALID_RESPONSE_XPATH, xmi, "");
		...
		int[] elems = XPathHelper.selectNodes(implNode, "ns:" + TAG_NAMESPACES
				+ "/ns:" + TAG_BINDING, xmi);
		...
		for (int node : elems) {
			...
			if (prefix == null) {
				throw new ConnectorException(
						ConnectorExceptionMessages.MISSING_ATTRIBUTE_PREFIX_FROM_NAMESPACE_BINDING_ELEMENT);
			}

			if ((uri == null) || (uri.length() == 0)) {
				throw new ConnectorException(
						ConnectorExceptionMessages.MISSING_ATTRIBUTE_URI_FROM_NAMESPACE_BINDING_ELEMENT);
			}
